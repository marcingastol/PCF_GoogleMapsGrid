...
export class GoogleMapsGrid implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  private loadGoogleMapsApi(): Promise<void> {
    if (GoogleMapsGrid._googleMapsScriptLoaded) {
      // Already loaded
      return GoogleMapsGrid._googleMapsInitPromise!;
    }
    ...
    GoogleMapsGrid._googleMapsInitPromise = new Promise((resolve, reject) => {
      script.onload = () => {
        resolve();
      };
  ...
  private renderMap(): void {
    ...
    mapDiv.style.height = "400px"; // adjust as needed
